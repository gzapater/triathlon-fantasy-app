...
Revision ID: 3e3e685925e0
Revises: 
Create Date: 2025-07-14 22:49:57.830962
...
def upgrade():
 # --- INICIO DE LA MODIFICACIÓN ---
    # Comentamos la creación de los ENUM para asumir que ya existen.
    # eventstatus_enum = sa.Enum('PENDIENTE', 'VALIDADO', 'RECHAZADO', name='eventstatus')
    # eventstatus_enum.create(op.get_bind())

    # racestatus_enum = sa.Enum('PLANNED', 'ACTIVE', 'ARCHIVED', name='racestatus')
    # racestatus_enum.create(op.get_bind())
    ...
    with op.batch_alter_table('leagues', schema=None) as batch_op:
        ...
        batch_op.create_index(batch_op.f('ix_leagues_name'), ['name'], unique=True)
...
def downgrade():
    ...
    op.drop_table('races')
    with op.batch_alter_table('leagues', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('ix_leagues_name'))
    ...
    op.execute('DROP TYPE IF EXISTS racestatus;')
